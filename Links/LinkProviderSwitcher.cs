namespace Spark.Sitecore.Links
{
	...
	/// </summary>
	public class LinkProviderSwitcher : LinkProvider
	{
		#region Fields
		/// <summary>
		/// The URL options.
		...
		#region Constructors
		/// <summary>
		/// Initializes a new instance of the <see cref="LinkProviderSwitcher"/> class.
		/// </summary>
		public LinkProviderSwitcher()
		{
		...
		public static SiteInfo FindMatchingSite(string itemFullPath, string itemLanguage)
		{
			...
			var sites = global::Sitecore.Configuration.Factory.GetSiteInfoList();
		...
		private static LinkProvider ResolveProvider(Item item, UrlOptions options)
		{
			...
			var provider = LinkProviderResolver.Resolve(siteName, item.Paths.FullPath, item.Language.Name);

			return provider ?? new LinkProvider();
		}
